...
class AppConfig:
    ...
    # File paths
    vector_store_path: str = field(default_factory=lambda: os.getenv("VECTOR_STORE_PATH", "data/indices/faiss_index.bin"))
    data_dir: str = "data"
    ...
    def _validate_config(self):
        ...
        if not (0.0 <= self.retrieval.similarity_threshold <= 1.0):
            raise ConfigurationError("retrieval.similarity_threshold must be between 0.0 and 1.0")
        
        if not (0.0 <= self.generation.temperature <= 2.0):
            raise ConfigurationError("generation.temperature must be between 0.0 and 2.0")
...
try:
    ...
    logging.getLogger(__name__).debug("Configuration not auto-loaded, waiting for explicit initialization")